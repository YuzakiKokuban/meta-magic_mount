...
ui_print "- Installing $ARCH_BINARY as meta-hybrid"

# Rename the selected binary to the generic name
mv "$MODPATH/bin/$ARCH_BINARY" "$MODPATH/meta-hybrid" || abort "! Failed to rename binary"

# Remove the unused binary
rm -rf "$MODPATH/bin"

# Ensure the binary is executable
chmod 755 "$MODPATH/meta-hybrid" || abort "! Failed to set permissions"

ui_print "- Architecture-specific binary installed successfully"

mkdir -p /data/adb/meta-hybrid

if [ ! -f /data/adb/meta-hybrid/config.conf ] ; then
  ui_print "- Add default config"
  cat "$MODPATH/config.conf" > /data/adb/meta-hybrid/config.conf
fi
